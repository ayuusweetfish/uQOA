...
// Unused throughout library code
static const uint16_t qoa_scalefactor_tab[16] = {
  1, 7, 21, 45, 84, 138, 211, 304, 421, 562, 731, 928, 1157, 1419, 1715, 2048
};
// dqt = {0.75, 2.5, 4.5, 7}
// qoa_dequant_tab[s][q] = round_away_0(scalefactor_tab[s] * dqt[q])
...
{
  // |v| <= 65535
  // Calculate n = v / qoa_scalefactor_tab[sf]
  // reciprocal[s] = ceil(32768 / scalefactor_tab[s])
  static const uint16_t reciprocal[16] = {
    32768, 4682, 1561, 729, 391, 238, 156, 108, 78, 59, 45, 36, 29, 24, 20, 16
  };
  int16_t n = (v * reciprocal[sf] + (1 << 14)) >> 15;
...
uint64_t qoa_encode_slice(qoa_lms *lms, int16_t samples[20], uint8_t *sf_hint)
{
  ...
  for (uint8_t sfi = 0; sfi < 16; sfi++) {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      int16_t quantized = qoa_quant(residual, sf);
      int16_t dequantized = qoa_dequant_tab[sf][quantized / 2];
      if (quantized % 2 == 1) dequantized = -dequantized;
      int16_t reconstructed = qoa_sat_s16(predicted + dequantized);

      int16_t error = sample - reconstructed;
      // printf("%2d %2d %6d %6d %1d %6d %d\n", sf, i, (int)sample, (int)predicted, (int)quantized, (int)reconstructed, (int)error);
